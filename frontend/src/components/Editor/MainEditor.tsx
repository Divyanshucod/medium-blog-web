...
export const MainEditor = (props:editorType)=>{
   const [togglePreview,setTogglePreview] = useState(props.readonly)
    return <div className="h-full rounded px-4 pb-4 bg-white relative">
      <Slate
        editor={props.editor}
        initialValue={props.blog}
        onChange={(value) => props.setBlog(value) }
      >
        {togglePreview ? <ToolBar isloading={props.isloading} handleClick={props.handleClick ?? (()=>{})} updateFunction={props.updateFunction} isUpdate={props.isUpdate}/>:null}
        <div className="h-full">
          <Editable
          readOnly={!togglePreview}
          name="Post"
          placeholder="Title"
          autoFocus
        ...
        <button onClick={() => setTogglePreview((prev) => !prev)} className="absolute right-2 bottom-2 rounded-sm shadow-sm">
           <Eye size={20}/>
        </button>
      </Slate>
    </div>
